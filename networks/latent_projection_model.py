...
from rfan.get_model import get_model
...
class ResAttnUNet_DS(nn.Module):
    ...
    def forward(self, x, ref) -> torch.Tensor:
        x0 = self.input_blocks(x)
        x1 = self.conv1_0(x0)
        x2 = self.conv2_0(x1)
        x3 = self.conv3_0(x2)
        ...
        x3_1 = self.conv3_1(torch.cat([x3, x4, res_b], dim=1))
        x2_2 = self.conv2_2(torch.cat([x2, x3_1, res_b], dim=1))
        x1_3 = self.conv1_3(torch.cat([x1, x2_2, res_b], dim=1))
        x0_4 = self.conv0_4(torch.cat([x0, x1_3, res_b], dim=1))
